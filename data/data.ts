const characterTestADEPT15 = [
  {
    title: 'Goal orientation',
    info: [
      'Focused on completing work and striving to succeed - as long as goals are clearly identified and within reach',
      'Willing to accept and deliver good, but not perfect, results in order to get things done',
      'Generally willing to reprioritize when facing setbacks at work',
      'That they usually complete work on time',
    ],
  },
  {
    title: 'Structure',
    info: [
      'That they can usually focus on both details and the big picture',
      'That they can approach tasks in a methodical and planned way when needed',
      'That they prefer to be organized, but tend to be less organized during stressful periods',
      'That they can produce structured plans when there is a need for it',
    ],
  },
  {
    title: 'Conceptual',
    info: [
      'That they are creative and intellectually interested within their areas of competence',
      'That they prefer tactical approaches, but are open to thinking strategically',
      'That they can find original solutions to common or familiar problems, but are less likely to do so with larger or more complex problems',
    ],
  },
  {
    title: 'Flexibility',
    info: [
      'Open regarding certain subjects, but rigid in their opinions and beliefs regarding others',
      'Willing to change and adapt as long as there is a good reason for it',
      'Open to new ideas and experiences, but do not actively seek them',
      'Flexible, but usually prefer that things do not change',
      'That they prefer well-proven methods, as long as they work reasonably well',
    ],
  },
  {
    title: 'Mastery',
    info: [
      'That they prefer to focus on existing strengths, but are also willing to develop new ones',
      'That they believe individual abilities are difficult to change, but certain skills can be developed through hard work, training and education',
      'Interested in helping talented individuals learn and develop',
    ],
  },
  {
    title: 'Ambition',
    info: [
      'Somewhat more interested in personal goals than career-related goals',
      'That they usually have a good balance between work and private life',
      'That they are somewhat more interested in enjoying their job rather than being promoted',
      'That they sometimes prefer achievable goals over very challenging goals at work',
    ],
  },
  {
    title: 'Influence',
    info: [
      'Good team players in most situations',
      'That they sometimes prefer to lead by example rather than through authority and responsibility',
      'That they are usually open to taking direction from others',
      'Somewhat more comfortable contributing as an individual than as a leader',
    ],
  },
  {
    title: 'Dominance',
    info: [
      'That they generally prefer a work environment characterized by collaboration rather than competition',
      'Rarely insult others and rarely behave aggressively',
      'Mostly cautious and thoughtful',
    ],
  },
  {
    title: 'Sociability',
    info: [
      'Mostly comfortable working independently without much social contact',
      'That they generally prefer to be alone or in small groups of close friends',
      'Somewhat less interested in large social events',
    ],
  },
  {
    title: 'Stability',
    info: [
      'Open about their passion and enthusiasm',
      'Quick to respond, easy to read and in touch with their feelings',
    ],
  },
  {
    title: 'Optimism',
    info: [
      'That they often evaluate situations carefully and are aware of potential problems',
      'Generally realistic in their promises',
      'That they can sometimes be self-critical and eager to perform well',
    ],
  },
  {
    title: 'Awareness',
    info: [
      'That they have a good understanding of how their behavior affects others',
      'Largely aware of their own strengths and weaknesses',
      'That they generally seek feedback',
    ],
  },
  {
    title: 'Cooperativeness',
    info: [
      'Willing to help others when needed for the team to succeed',
      'That they usually trust others, but are somewhat skeptical at first',
      'Open to asking for help, but prefer to avoid it if possible',
      'Mostly willing to accommodate others',
    ],
  },
  {
    title: 'Considerateness',
    info: [
      'Overall sympathetic, supportive and interested in the problems of others',
      'That they usually prefer to acquit rather than condemn',
      'Usually interested in understanding the behavior of individuals',
    ],
  },
  {
    title: 'Modesty',
    info: [
      'Generally sincere, open and honest',
      'Usually hesitant to take credit for successes',
    ],
  },
];

const testimonials = [
  {
    name: 'Sara Eriksson',
    image:
      'https://images.unsplash.com/photo-1488508872907-592763824245?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80',
    position: 'COO, Gruppera',
    text: 'Jimmy är enkel att arbeta med, han är nyfiken och drar sig intefrån att fråga om något är oklart. Han är social och bidrar till god stämning på arbetsplatsen och flyttar med enkelhet fokus till rätt sak för tillfället.',
  },
  {
    name: 'Robert Gustafsson',
    position: 'Kontorschef, Grade',
    image:
      'https://images.unsplash.com/photo-1488508872907-592763824245?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80',
    text: 'Jimmy har visat sig nyfiken och analytisk. Han har också visat ett stort intresse kring att förstå hela bilden för att kunna leverera en så bra produkt som möjligt.',
  },
];

const testCertificates = [
  // {
  //   slug: 'accessibility',
  //   title: 'Accessibility',
  //   subTitle: 'WCAG2.1',
  //   infoText:
  //     'Did you know WCAG2.1 has its own fan club? They are called "Wcaggers", and they are just as passionate about accessibility as your grandma is about knitting.',
  // },
  {
    slug: 'cpr',
    title: 'cpr',
    subTitle: 'First Aid',
    infoText:
      'CPR instructors are basically the DJs of the medical world - they know all the beats to keep the heart pumping, and they all even throw in some "Stayin Alive" by the Bee Gees for good measure.',
  },
  // {
  //   slug: 'netflix-navigating',
  //   title: 'Netflix Navigating',
  //   subTitle: 'Ninja',
  //   infoText:
  //     'To earn the prestigious "Netflix Navigating Ninja" certificate, binge-watch at least 20 series in a week without falling asleep or losing the remote control. Bonus points for perfecting the art of ordering takeout without leaving your couch!',
  // },
  // {
  //   slug: 'cooking',
  //   title: 'Cooking',
  //   subTitle: 'Microwave Magician',
  //   infoText:
  //     'Microwave Magicians have turned reheating leftovers into a culinary art form, defying expectations one beep at a time.',
  // },
  // {
  //   slug: 'texting',
  //   title: 'Texting',
  //   subTitle: 'Emoji Ambassador',
  //   infoText:
  //     'Emoji Ambassadors have unlocked the secret language of emojis, fluently expressing emotions and ideas through the art of tiny digital symbols.',
  // },
];

const testAboutData = [
  {
    href: 'about/jimmy',
    title: 'Jimmy',
    text: "This narrative recounts a diverse and meaningful life journey. It begins with a transformative experience as a Buddhist monk in Thailand, reflecting on the cultural tradition of seeking ordination to apologize to parents and express gratitude. The narrative then shifts to the author's educational journey, where they graduated in 2019, driven by a desire to create tech applications that solve real-world problems and contribute positively. Lastly, the author highlights their love for the outdoors, dedication to physical fitness through calisthenics, and the importance of family and friends in maintaining a balanced life.",
  },
  {
    href: 'about/adept',
    title: 'Adept',
    text: 'The text describes the results of a test that measures various personality traits. The test provides insight into different aspects of my personality and reveals my behavioral patterns and work style. By understanding and reflecting on these results, I can become aware of my strengths and weaknesses and utilize them to improve my performance and adapt effectively to the work environment',
  },
  {
    href: 'about/testimonial',
    title: 'Testimonials',
    text: "Discover a collection of inspiring testimonials I've received from my former colleagues and bosses.",
  },
  {
    href: 'about/certificates',
    title: 'Certificates',
    text: "Visit my certificate collection, where new qualifications magically appear like rabbits from a magician's hat – you never know what impressive trick I'll pull off next! Keep checking in, it's like unwrapping surprise gifts of expertise.",
  },
];

const testEpigramData = [
  {
    href: '#',
    title: 'Stay afraid',
    text: 'Stay afraid, but do it anyway. What’s important is the action. You don’t have to wait to be confident. Just do it and eventually the confidence will follow.',
  },
  {
    href: '#',
    title: 'Tide will turn',
    text: 'When you get into a tight place and everything goes against you…never give up then, for that is just the place and time that the tide will turn.',
  },
  {
    href: '#',
    title: 'Distance Traveled',
    text: 'We should not judge people by their peak of excellence; but by the distance they have traveled from the point where they started.',
  },
  {
    href: '#',
    title: 'Crazy enough',
    text: 'People who are crazy enough to think they can change the world are the ones who do.',
  },
];

const testDataJimmyCard = [
  {
    title: 'Great outdoors',
    text: "I find solace in the great outdoors. There's nothing quite like setting up my 4-season tent in the midst of a winter wilderness and simply being one with nature.",
    image: '/camping.jpeg',
  },
  {
    title: 'Calisthenics',
    text: 'I also have a passion for calisthenics, pushing my physical limits to achieve personal growth.',
    image: '/pullup.jpg',
  },
  {
    title: 'Cold bath',
    text: 'Taking cold baths when the opportunity arises is another invigorating pursuit.',
    image: '/iceBath.jpeg',
  },
  {
    title: 'Family',
    text: 'However, my heart belongs to my family, which includes my wife and two older children. They are my anchor, and  I relish every moment spent with them.',
    image: '/family.jpeg',
  },
  {
    title: 'Friends',
    text: 'I cherish the times I spend with friends, often meeting up at a restaurant for a hearty burger and a refreshing drink.',
    image: '/friends.jpeg',
  },
  {
    title: 'Balance',
    text: "It's in these moments that I truly appreciate the balance of my life, from introspective solitude to the joy of companionship.",
    image: '/balance.webp',
  },
];

const quotesTEstData = [
  {
    quote:
      'There are two mistakes along the way to Self-Mastery: Not starting it and not going all the way.',
    author: 'Shi Heng Yi',
  },
  {
    quote: 'Freedom goes hand in hand with your ability to restrict yourself',
    author: 'Shi Heng Yi',
  },
  {
    quote:
      'It’s not important what you mean by your words. It’s important how the other person understands what you just said.',
    author: 'Shi Heng Yi',
  },
  {
    quote:
      'To regain control, we must train the mind to be more present, to live in the here and now, and to find the answers within ourselves, to perceive every moment as it is, and feel the subtle changes within, to continuously work on it. That is the essential practice.',
    author: 'Shi Heng Yi',
  },
];

const testDeathNoteRules = [
  {
    heading: 'Entering Names',
    text: [
      "The human whose name is written in this note shall die. This note will not take effect unless the writer has the subject's face in mind when writing his/her name. This is to prevent people who share the same name from being affected.",
      'The Death Note will not ever affect a victim whose name has been misspelled four times.',
      'When the same name is written in two or more Death Notes, the Note which was used first will take effect, regardless of the time of death.',
      'If the same name is written in two or more Death Notes within 0.06 seconds, the entry is regarded as simultaneous; the Death Notes will not take effect and the individual will not die.',
      "If a Death Note owner accidentally misspells a person's name four times, that person will be free from being killed by the Death Note. However, if the Death Note owner intentionally misspells the name four times, the owner will die.",
      "The Death Note will not take effect if a victim's name is written on several different pages. However, the front and back of a page is considered to be one page. For example, the Death Note will still take effect if the victim's last name is written on the front page and first name on the back.",
    ],
  },

  {
    heading: 'Entering the Causes and Conditions of Death',
    text: [
      "If the cause of death is written within 40 seconds of the subject's name, it will happen.",

      'If the cause of death is not specified, the subject will die of a heart attack.',

      'After writing the cause of death, the details of death should be entered within the next six minutes and 40 seconds.',

      'If the time of death is specified within 40 seconds after writing the cause of death as a heart attack, the time of death can be manipulated and can go into effect within 40 seconds after writing the name.',

      'The conditions of death will not be realized unless they are physically possible for that human or could be reasonably assumed to be carried out by that human.',

      'Since the limitations applying to the conditions of a death are unknown to the Shinigami, Death Note owners must find out on their own.',

      'You may write the cause and/or details of death prior to filling in the name of the individual. Be sure to insert the name in front of the cause of death. You have about 19 days (according to the human calendar) to fill in a name.',

      'Suicide is a universally valid cause of death as all humans are thought to possess the potential to commit suicide. It is, therefore, something that may be reasonably assumed of an individual.',

      "Whether the cause of the individual's death is either suicide or an accident, if it would lead to the death of more than the intended victim, the person will simply die of a heart attack. This is to ensure that other lives are not impacted.",

      "After an individual's name, time of death, and conditions of death are entered in the notebook, the time and conditions of death may be altered as many times as desired as long as they are changed within six minutes and 40 seconds from the time they are filled in. But, of course, this is only possible before the victim dies.",

      "If you write 'dies of accident' for the cause of death, the victim will die from a natural accident six minutes and 40 seconds after the time of entry.",

      "Even if only one name is written in the Death Note, if the victim's death causes other humans that are not written in it to die, the cause of death will default to a heart attack.",

      "If you write 'die from disease' and specify which disease and the time of death, there must be a sufficient amount of time for the disease to progress. If the set time is too tight, the victim will die of a heart attack six minutes and 40 seconds after the entry in the Death Note.",

      "If you write, 'dies from disease' as the cause of death but specify only a time of death and not the actual disease, the victim will die from a plausible disease.",

      'The Death Note can only operate within a 23-day window (in the human calendar). This is called the 23-Day rule.',

      "If you write 'dies from disease' and specify which disease but not a time of death, if the progression of the disease takes more than 24 days, the 23-Day rules will not take effect and the human will die at an appropriate time depending on the disease. However, rewriting the cause and/or details of death must be done within six minutes and 40 seconds: you cannot change the victim's time of death, however soon it may be.",

      "In order for the Death Note to take effect the victim's name must be written on one page; however, the cause and conditions of death may be entered on other pages. This will work as long as the person who writes in the Death Note keeps the specific victim's name in mind when writing the cause and conditions of death.",

      "If the cause and conditions of death are written in before the victim's name is, multiple names can be written as long as they are entered within 40 seconds and the cause and conditions of death are not impossible. In the event that the cause of death is possible but the conditions are not, only the cause of death will take effect for that victim. If both the cause and the conditions are impossible, that victim will die of a heart attack.",

      "When you write multiple names in the Death Note and then write down one cause of death within 40 seconds of writing the first victim's name, the cause will take effect for all the written names. Also, after writing the cause of death, even if the conditions of death are written within six minutes and 40 seconds in the human world, the conditions will apply only to the victims for whom they are possible. Those for whom the conditions are not possible will simple die from the specified cause.",

      "A human death caused by the Death Note can indirectly lengthen another human's original life span in the human world even without the owner's specific intention to do so.",

      "Once the victim's name, cause of death, and conditions of death have been written down in the Death Note, the death will take place even if that Death Note, or the part of the note used, is destroyed before the stated time of death.",

      "If the victim's name has been entered and the Death Note is destroyed while the cause of death is being written, the victim will be killed by a heart attack 40 seconds after the name was entered. If the victim's name and cause of death have already been written, then the victim will be killed within six minutes and 40 seconds via the stated cause of death if it is possible within that period of time. Otherwise, the victim will die by heart attack.",
    ],
  },
  {
    heading: 'Alterations',
    text: [
      "If you wish to change anything written in the Death Note within six minutes and 40 seconds after you wrote it, you must first rule out the characters you want to erase with two straight lines. The time and conditions of death can be changed, but once the victim's name has been written, that individual's death can never be averted.",

      'It is useless to try to erase names written in the Death Note with erasers or to white them out.',

      "Even if a new victim's name, cause of death, or conditions of death are written on top of the original victim's name, cause of death, or conditions of death, there will be no effect on the original victim's death. The same thing will also apply to erasing what was written with a pencil, or whiting out what was written with a pen.",
    ],
  },
  {
    heading: 'Limitations',
    text: [
      'The Death Note will not affect those less than 780 days old.',

      'You cannot kill humans who are more than 124 years of age with the Death Note.',

      'You cannot kill humans with less than 12 minutes of life left (in human calculations).',

      "You cannot set a death date longer than the victim's original life span. Even if the victim's death is entered in the Death Note, if it is beyond his or her original life span, the victim will die before the set time.",
    ],
  },
  {
    heading: 'Other Things to Consider',
    text: [
      'The human who uses this note can go neither to Heaven nor to Hell.',

      'One page taken from the Death Note, or even a fragment of the page, possesses the full power of the note.',

      'Any writing instrument or medium (cosmetics, blood, etc.) may be used, as long as it can write directly onto the note and create legible text.',

      'The pages of the Death Note will never run out.',

      'Some limited number of Death Notes have white or red front covers, but this makes no different in their effectiveness as compared with the black Death Notes.',
    ],
  },
  {
    heading: 'The Beginning of Ownership',
    text: [
      'This note shall become the property of the human world once it touches ground in the human world.',

      'The owner of a Death Note can recognize the image and voice of the original owner - a Shinigami, for example.',

      'The human who touches the Death Note can recognize the image and voice of its Shinigami owner, even if the human is not the owner of the note.',

      "Whenever a Shinigami in the human world dies and leaves behind its Death Note, the note's finder automatically becomes the owner. However, in this case, only a human who can see and hear that Shinigami is able to see and touch the Death Note. It is very unlikely, but if by any chance another Shinigami picks up the Death Note, that Shinigami becomes the owner.",

      'If you lose the Death Note or have it stolen, you will lose its ownership unless you can retrieve it within 490 days.',

      'When the owner of the Death Note dies while the note is on loan, its ownership will be transferred to the person who is holding it at that time. If the Death Note is stolen and the owner is killed by the thief, its ownership will automatically be transferred to the thief.',

      'The individuals who lose ownership of a Death Note will also lose their memories of it. However, this does not mean that they will lose all memories from the period of ownership: they will only lose the memories involving the Death Note.',

      "When an individual with ownership of more than two Death Notes loses possession of one of them, he will no longer be able to recognize or hear that Death Note's Shinigami anymore. The Shinigami will leave, but all the memories involving the Death Note will remain to the owner as long as he maintains ownership of at least one other Death Note.",

      "If a person loses possession of a Death Note he will not recognize its Shinigami by sight or voice anymore. However, if the owner lets someone else touch that Death Note, from that time on that person will continue to recognize the Shinigami's appearance and voice until he or she actually becomes the owner of the Death Note and subsequently loses possession of it.",

      'When regaining ownership of a Death Note, the memories associated with it will also return. In cases where the owner was involved with other Death Notes as well, memories of all the Death Notes involved will return. The memories will return just by touching the Death Note, even without obtaining ownership of it.',

      "Memories related to a Death Note are lost when its ownership is lost. But they may be regained by either obtaining ownership once again or by touching the Death Note. This can be done up to six times per Death Note. Any times more than that, the person's memory of the Death Note will not return and they will have to use it without any previous memory of it.",

      'Even if you do not actually possess the Death Note, you may still use it to full effect.',

      'You may lend the Death Note to another person while maintaining its ownership. The borrower may lend it to yet another person as well.',

      'The person who borrows the Death Note will not be followed by a Shinigami. The Shinigami always remains with the owner of the Death Note. Also, the borrower cannot trade for the Shinigami Eyes.',

      "Only by touching each other's Death Notes can owners recognize the appearance or voice of each other's Shinigami.",

      "Losing memory of the Death Note by passing the ownership to another or by abandoning ownership will only occur when someone is actually killed using that Death Note. You will not lose memory of the Death Note if, for example, you merely owned it and did not write down anyone's name. In this case, you will not be able to hear or see the Shinigami anymore. You will also lose the power of the Shinigami Eyes if you made the trade.",
    ],
  },
  {
    heading: 'Multiple Notebooks',
    text: [
      "Someone possessing more than one Death Note may write down a victim's name in one of the Death Notes and the cause of death in the other, and the death will still occur. The order of writing, however, is unimportant: if you write down the cause of death in one Death Note and afterward write the name in the other, the death will occur. This can be accomplished by two Death Note owners working together. In this case, it's necessary that the two touch each other's Death Notes.",

      'Only six Death Notes are allowed to exist at a time in the human world. Of course, the Death Notes that the Shinigami own do not count. This means only six Shinigami that have passed on their Death Notes to humans can be in the human world at once.',

      'One Shinigami is allowed to pass on Death Notes to only three humans at a time. However, it is possible for a single Shinigami to hand out up to six Death Notes - by handing three humans two Death Notes each, for example. In other words, one human could potentially own all six Death Notes.',

      'If a seventh Death Note is given to a human when six already exist in the human world, nothing will happen when it is used.',

      'In the event that there are more than six Death Notes in the human world, only the first six Death Notes that have been delivered to humans will have effect. The seventh Death Note will not become active until one of the other six Death Notes is destroyed or a Shinigami takes one of them back to the Shinigami realm.',
    ],
  },
  {
    heading: 'The Shinigami Eyes',
    text: [
      "A human who becomes the owner of a Death Note can, in exchange for half of his or her remaining life, acquire the power of the Shinigami Eyes, which will enable him or her to see a human's name and remaining life span when looking at them.",

      "An individual with Shinigami Eyes can see the name and life span of another human by looking at that person's face. By gaining ownership of a Death Note, an individual not only gains the ability to kill but also cannot be killed by a Death Note. From this point on, a person with a Death Note cannot see the life span of other Death Note owners, including him- or herself.",

      "In order to see the names and life spans of humans using the power of the Shinigami Eyes, the owner must be able to see more than half of that person's face. When looking from top to bottom, he must be able to see at least from the head to the nose. If he looks at only the eyes and under, he will not be able to see the person's name and life span. Also, even though some parts of the face - for example the eyes, nose or mouth - are hidden, if he can basically see the whole face he will be able to see the person's name and life span.",

      'It is still not clear how much exposure is needed to see a name and life span (more research needs to be done). If the above conditions are met, names and life spans can be seen using photos and digital images, no matter how old they are. But this is sometimes affected by the resolution and size of the image. Also, names and life spans cannot be seen using drawings, however realistic they may be.',

      'Those with Shinigami Eyes will have eyesight of over 3.6 in the human measurement, regardless of their original eyesight.',

      "If you have traded for the Shinigami Eyes, you will see a person's primary life span in the human world.",

      "The names you will see with the Shinigami Eyes are the names needed to kill that person. You will be able to see a name even if it isn't officially registered anywhere.",

      'Humans who have traded for Shinigami Eyes cannot see the names or life spans of humans who have already passed away (by looking at photos of them, for example).',

      "The use of the Death Note in the human world sometimes affects other humans' lives or shortens their original life spans, even if their names are not actually written in the notebook itself. In these cases, no matter the cause, the Shinigami sees only the original life span and not the shortened life span.",

      'No matter what medical or scientific method is employed, it is impossible for humans to distinguish whether or not a human has Shinigami Eyes. Even Shinigami cannot distinguish this fact, except for the very Shinigami that traded his or her eye power with that human.',
    ],
  },
  {
    heading: 'The Shinigami Rules',
    text: [
      'The human owner of a Death Note is possessed by its original Shinigami owner until he or she dies.',

      'If a human uses a Death Note, its Shinigami owner must appear in front of the human within 39 days after he or she uses the note.',

      "The original Shinigami owners of Death Notes don't, in principle, do anything to help or prevent the deaths brought about by the notes.",

      'A Shinigami has no obligation to completely explain how to use the note or the rules that apply to the human who owns it.',

      'The Shinigami must not tell humans the names or life spans of individuals he sees. This is to avoid confusion in the human world.',

      'A Shinigami bringing a Death Note into the human world must make sure that a human uses it. Although it is unlikely that a Shinigami who has possessed a human would die, if it does happen, the Death Note brought into the human world will not lose its power.',

      'Shinigami must not stay in the human world without a particular reason. Acceptable reasons to stay in the human world are as follows:',

      "I. When the Shinigami's Death Note is handed to a human.",
      'II. Finding a human to take possession of a Death Note should be done from the Shinigami realm, but if it is within 82 hours this may also be done in the human world.',
      'III. When a Shinigami stalks an individual with an intention to kill them, as long as it is within 82 hours of possessing them the Shinigami may stay in the human world.',

      'The Shinigami must not hand the Death Note directly to a child under six years of age (based on the human calendar). But Death Notes that have been dropped into the human world, and are part of the human world, can be used upon humans of almost any age with the same effect.',

      'The owner of a Death Note cannot be killed by a Shinigami who is in the Shinigami realm. Also, a Shinigami who comes to the human world with the objective of killing the owner of a Death Note will not be able to do so. Only a Shinigami that has passed on its Death Note to a human is able to kill the owner of the Death Note.',

      "If a Death Note is owned in the human world against a Shinigami's will, that Shinigami is permitted to stay in the human world in order to retrieve it. In that case, if there are other Death Notes in the human world, the Shinigami are not allowed to reveal to the humans the Death Note owner's identity or its location.",

      "If a Shinigami's Death Note is taken away for whatever reason, it can only be retrieved from the Shinigami who possesses it at the time. If there is no Shinigami, but a human, the only way the Shinigami can get it back is to first touch the Death Note and become the one who haunts that particular human. Then they have to wait until that person dies to take it away. And they have to do it before any other human touches it.",

      'After a Shinigami brings a Death Note to the human world and gives its ownership to a human, the Shinigami has the right to kill the human using its own Death Note for any reason, such as disliking the owner.',

      'The following are cases in which a Shinigami that has brought the Death Note into the human world is allowed to return to the Shinigami realm:',

      "I. When the Shinigami has seen the end of the first owner of the Death Note brought into the human world and has written that human's name into his or her own Death Note.",
      'II. When the Death Note is destroyed and cannot be used by humans anymore.',
      'III. If nobody claims ownership of the Death Note, it is unnecessary to possess anyone.',
      'IV. If, for any reason, the Shinigami possessing the Death Note is replaced by another Shinigami.',
      'V. When a Shinigami loses track of the Death Note that he or she possesses, or cannot identify which human owns the Death Note, or cannot locate where the owner is, and therefore needs to find such information using the observation holes in the Shinigami realm.',
      "VI. Even in cases II, III, and IV, Shinigami are obliged to confirm the death of the first owner and write down that human's name in his or her Death Note even when he or she is in the Shinigami realm.",

      "In the Shinigami realm there are a few copies of what humans might call a 'user handbook' for the Death Notes in the human world. Although Shinigami may not give the handbook to humans, it is perfectly okay for them to teach humans about its contents, no matter what that may be.",
    ],
  },
  {
    heading: 'Using the Death Note',
    text: [
      'A Shinigami can extend its life by writing human names in a Death Note, but a human cannot. A person can only shorten his or her life by using the note.',

      'Even the original Shinigami owners of Death Notes do not know much about them.',

      'Shinigami must own at least one personal Death Note, which must never be lent to or written on by a human.',

      "Shinigami may exchange and write in each others' Death Notes.",

      "If a Shinigami decides to use a Death Note to end the life of the killer of an individual it favors, that individual's life will be extended but the Shinigami will die. The Shinigami will disappear but the Death Note will remain. The ownership of this Death Note is usually carried over to the next Shinigami that touches it, but it is common sense that it be returned to the Shinigami King.",

      "By manipulating the death of a human who has influence over another human's life, that human's original life span can sometimes be lengthened. If a Shinigami intentionall does this that Shinigami will die, but even if a human does the same, the human will not die.",
    ],
  },
  {
    heading: 'Shinigami Life',
    text: [
      'A Shinigami cannot be killed even if stabbed or shot. However, there are ways to kill them, which are not generally known even to the Shinigami themselves.',

      'There are male and female Shinigami, but it is neither permitted nor possible for them to have sexual relations with humans. They also cannot have sex with each other.',

      'As long as a Shinigami has at least once seen a human and knows his or her name and life span, the Shinigami is capable of finding that human by looking down from an observation hole in their realm.',

      'The Shinigami realm has laws that govern it. If a Shinigami should break a law, there are nine levels of punishment, which the severity starting at Level Eight and going up to Level One, plus an Extreme Level. At severity levels above Three, the Shinigami will be punished and killed. Killing a human without using the Death Note merits punishment at the Extreme Level.',

      "Shinigami will not die from lack of sleep. They do not need sleep, so to them it is merely laziness. Shinigami in the human world shouldn't act lazy merely because they are required to possess a human.",
    ],
  },
  {
    heading: 'Fake Rules',
    text: [
      'If you make this note unusable by tearing it up or burning it, all humans who have touched the note until then will die.',

      'If the person using the note fails to consecutively write names of people to be killed within 13 days, then the user will die.',
    ],
  },
];

const testDataJapaneseZodiacSigns = [
  {
    englishSign: 'Rat',
    japaneseSign: '子',
    animalKanji: '鼠',
    birthYears: [
      '1924',
      '1936',
      '1948',
      '1960',
      '1972',
      '1984',
      '1996',
      '2008',
      '2020',
    ],
    positiveAttributes: ['outgoing', 'energetic', 'resourceful'],
    negativeAttributes: ['rude', 'greedy'],
    info: [
      'As number one in the zodiac, the Rat has some outstanding qualities. According to the myth, the clever Rat hitched a ride on the Ox and leapt over the finish line to become the first place winner.',

      'The Rat embodies luck and success, and so a Rat year is said to be a great one for fresh starts. While a Rat person can be extremely capable and intelligent, they may also be hard to read and have a miserly streak.',
    ],
  },
  {
    englishSign: 'Ox',
    japaneseSign: '丑',
    animalKanji: '牛',
    birthYears: [
      '1925',
      '1937',
      '1949',
      '1961',
      '1973',
      '1985',
      '1997',
      '2009',
      '2021',
    ],
    positiveAttributes: ['hardworking', 'earnest', 'trustworthy'],
    negativeAttributes: ['narrow-minded', 'constrained'],
    info: [
      "With Asia's heavily agrarian history, it's no surprise that many of its cultures find significance in the ever-useful Ox.",

      "The Ox spurns laziness and understands the power of combining patience with hard work. However, Ox people are also quite stubborn and want to remain on the path they've chosen, and so it may be hard to persuade them to change their mind or consider an alternative point of view.",
    ],
  },
  {
    englishSign: 'Tiger',
    japaneseSign: '寅',
    animalKanji: '虎',
    birthYears: [
      '1926',
      '1938',
      '1950',
      '1962',
      '1974',
      '1986',
      '1998',
      '2010',
      '2022',
    ],
    positiveAttributes: ['brave', 'confident', 'passionate'],
    negativeAttributes: ['temperamental', 'impulsive'],
    info: [
      'The ferocious Tiger is a highly revered animal in East Asian culture, often depicted as a creature rivaling or just secondary to the mighty Dragon.',

      'A Tiger person will get things done with vigor and wit. They aren’t afraid to show how much they care for others, and so have great potential as leaders. However, they tend to have “up-and-down” and unpredictable personalities that can cause issues in their life and relationships.',
    ],
  },
  {
    englishSign: 'Rabbit',
    japaneseSign: '卯',
    animalKanji: '兎',
    birthYears: [
      '1927',
      '1939',
      '1951',
      '1963',
      '1975',
      '1987',
      '1999',
      '2011',
      '2023',
    ],
    positiveAttributes: ['elegant', 'talented', 'tactful'],
    negativeAttributes: ['gullible', 'bendable will'],
    info: [
      'Those born in the Year of the Rabbit can often be described as admirable and progress-driven people.',

      'You could certainly trust a Rabbit person to carry out a task with poise. On the other hand, they could be easily deceived or find themselves falling victim to unscrupulous types.',
    ],
  },
  {
    englishSign: 'Dragon',
    japaneseSign: '辰',
    animalKanji: '竜',
    birthYears: [
      '1928',
      '1940',
      '1952',
      '1964',
      '1976',
      '1988',
      '2000',
      '2012',
      '2024',
    ],
    positiveAttributes: ['steadfast', 'noble', 'brave'],
    negativeAttributes: ['short-tempered', 'uncontrollable'],
    info: [
      'The Dragon is an incredibly important creature in Asian mythology that embodies otherworldly grace and strength. In fact, according to the origin myth of the zodiac, the Dragon did not finish first because it chose to help out some of the others during the race instead.',

      'A Dragon person is likely to be charismatic, unique and inspiring due to their unwavering nature. However, as is typical of the giant lizards of lore, they’re prone to losing control of their moods and decisions.',
    ],
  },
  {
    englishSign: 'Snake',
    japaneseSign: '巳',
    animalKanji: '蛇',
    birthYears: [
      '1929',
      '1941',
      '1953',
      '1965',
      '1977',
      '1989',
      '2001',
      '2013',
      '2025',
    ],
    positiveAttributes: ['observant', 'wise', 'persistent'],
    negativeAttributes: ['indifferent', 'negatively introverted'],
    info: [
      'While Western culture often gives snakes a bad rap, Asian culture is much kinder on these sleek, elegant creatures. In the myth, the Snake realized its physical shortcomings and hitched a ride on the Horse, remaining unseen until the finish line.',

      'Snake individuals are said to be intelligent and thoughtful, as well as particularly suave with money matters. But the Snake can also seem like a cold individual, especially since they aren’t inclined to share their opinions and true feelings with others.',
    ],
  },
  {
    englishSign: 'Horse',
    japaneseSign: '午',
    animalKanji: '馬',
    birthYears: [
      '1930',
      '1942',
      '1954',
      '1966',
      '1978',
      '1990',
      '2002',
      '2014',
      '2026',
    ],
    positiveAttributes: ['sociable', 'charming', 'determined'],
    negativeAttributes: ['impatient', 'inconsiderate'],
    info: [
      'Much like their animal namesake, a Horse person is spirited and loves to be on the move—whether on a personal project or a new social trend. They ooze enough charisma to win them hordes of admirers.',

      'On the flip side, the Horse can get bored easily and become quickly demotivated when things don’t pan out their way. Their sociable nature can also come across as intrusiveness at times.',
    ],
  },
  {
    englishSign: 'Sheep',
    japaneseSign: '未',
    animalKanji: '羊',
    birthYears: [
      '1931',
      '1943',
      '1955',
      '1967',
      '1979',
      '1991',
      '2003',
      '2015',
      '2027',
    ],
    positiveAttributes: ['creative', 'kind', 'considerate', 'passionate'],
    negativeAttributes: ['indecisive', 'overly meek'],
    info: [
      'Interestingly, the Japanese zodiac often considers the Sheep as least compatible with the Ox, despite their animal counterparts both being four-legged agricultural livestock.',

      'Like the animal, the Sheep is said to be peace-loving and group-focused, as well as artistically gifted. But while obedience is a desirable trait in the actual animal, a Sheep person may be perceived as overly uncertain about themselves. ',
    ],
  },
  {
    englishSign: 'Monkey',
    japaneseSign: '申',
    animalKanji: '猿',
    birthYears: [
      '1932',
      '1944',
      '1956',
      '1968',
      '1980',
      '1992',
      '2004',
      '2016',
      '2028',
    ],
    positiveAttributes: ['clever', 'versatile', 'resourceful'],
    negativeAttributes: ['manipulative', 'short-sighted'],
    info: [
      'East Asian cultures have a special reverence for monkeys. They’re often considered smart and smooth—creatures who think as fast as they move.',

      'The zodiac Monkey is one of the most likely of the 12 zodiac animals to get out of a pinch as needed. However, the Monkey can also be arrogant and erratic, and doesn’t always consider the consequences of their actions. A Monkey is particularly compatible with other clever members of the zodiac, including the Rat and the Snake.',
    ],
  },
  {
    englishSign: 'Rooster',
    japaneseSign: '酉',
    animalKanji: '鶏',
    birthYears: [
      '1933',
      '1945',
      '1957',
      '1969',
      '1981',
      '1993',
      '2005',
      '2017',
      '2029',
    ],
    positiveAttributes: ['diligent', 'devoted', 'independent'],
    negativeAttributes: ['overambitious', 'aggressive'],
    info: [
      'A Rooster individual enjoys being busy, takes pride in their efforts and doesn’t back down from challenges or speaking their mind. They may get a bit too confident or ambitious, however, which can leave them crushingly disappointed if they fail to achieve their goal.',
    ],
  },
  {
    englishSign: 'Dog',
    japaneseSign: '戌',
    animalKanji: '犬',
    birthYears: [
      '1934',
      '1946',
      '1958',
      '1970',
      '1982',
      '1994',
      '2006',
      '2018',
      '2030',
    ],
    positiveAttributes: ['loyal', 'honest', 'down-to-earth'],
    negativeAttributes: ['stubborn', 'overly critical'],
    info: [
      'Loyalty is the hallmark of man’s best friend, and the zodiac Dog shares this trait. The Dog is often said to be the most faithful and dutiful of the zodiac members.',

      'A Dog individual may be cautious when forming relationships, but once they manage to create a bond, they will cherish it to the best of their ability. The Dog may struggle with giving or taking criticism, and their occasional obstinacy can make them difficult to deal with at times.',
    ],
  },
  {
    englishSign: 'Boar',
    japaneseSign: '亥',
    animalKanji: '猪',
    birthYears: [
      '1935',
      '1947',
      '1959',
      '1971',
      '1983',
      '1995',
      '2007',
      '2019',
      '2031',
    ],
    positiveAttributes: ['self-empowered', 'honest', 'humble'],
    negativeAttributes: ['short-tempered', 'naïve'],
    info: [
      'Some variations of the zodiac myth say the Boar was last due to the creature’s inherent laziness. Others say it was due to overcompensation—in one version, the Boar literally ran past the overseeing god and had to circle back.',

      'A Boar person possesses admirable inner strength, vitality and honesty—all qualities that make for good, sincere companions. They can be quick to anger and immovable in their judgments, though, particularly if you find yourself on their bad side.',
    ],
  },
];

const chantbookPhraPhutthakun = [
  {
    title: "วิธีทำสมาธิแบบง่าย ๆ",
    titleRomanized: "Withi Tham Samathi Bæp Ngai Ngai",
    description: `
    This chapter provides a simple and practical method for mindfulness meditation.
    It explains how to sit, where to place attention (on the rising and falling of the abdomen),
    and how to handle physical discomfort, wandering thoughts, and emotions using a noting technique.
    It emphasizes consistent awareness during both formal sitting and everyday activities,
    forming the foundation of insight meditation (vipassanā).
  `,
    paragraphs: [
      {
        thai: "ทำสมาธิเพื่อให้จิตสงบ มีพลัง มีประโยชน์ ในปัจจุบัน คือ ทำให้ใจสบาย คลายทุกข์ หนักแน่น มั่นคง อารมณ์แจ่มใส ความจำดี ทำงานมีประสิทธิภาพ สุขภาพดี นอนหลับสบาย เรียนหนังสือเก่ง ที่สำคัญคือได้บุญมาก",
        romanized:
          "Tham samathi phuea hai chit sangop mi phalang mi prayot nai patjuban khue tham hai chai sabai khlai thuk naknaen mankhong arom jaem sai khwamjam di thamngan mi prasitthiphap sukhaphap di nonlap sabai rian nangsue keng thi samkhan khue dai bun mak",
        eng: "Practice meditation to calm the mind, build strength, and bring practical benefits in everyday life — such as easing suffering, strengthening mental stability, brightening emotions, improving memory, increasing work efficiency, promoting health, better sleep, and success in studies. Most importantly, it brings great merit.",
      },
      {
        thai: "วิธีนั่งให้นั่งขัดสมาธิ คือ ขาขวาทับขาซ้าย นั่งตัวตรง หลับตา เอาสติมาจับอยู่ที่สะดือ ที่ท้องพองยุบ...",
        romanized:
          "Withi nang hai nang khat samathi khue kha khwa thap kha sai nang tua trong lap ta ao sati ma chap yu thi sadu thi thong phong yub...",
        eng: "Sit cross-legged with the right leg over the left, keep your back straight, and close your eyes. Bring awareness to the navel area and observe the rising and falling of the abdomen.",
      },
      {
        thai: "ในขณะที่นั่งหรือเดินจงกรมอยู่นั้น ถ้ามีเวทนา ความเจ็บปวด เมื่อย คัน เกิดขึ้น ให้หยุดเดิน หรือหยุดกำหนดพองยุบ...",
        romanized:
          "Nai khana thi nang rue doen jongkrom yu nan tha mi wethana khwam chep puat mueai khan koet khuen hai yut doen rue yut kamnot phong yub...",
        eng: "While sitting or walking in meditation, if painful sensations like aches or itching arise, stop walking or stop the 'rising-falling' noting. Bring mindfulness to the sensation and observe it as it is.",
      },
      {
        thai: "จิตเวลานั่งอยู่หรือเดินอยู่ ถ้าจิตคิดถึงบ้าน คิดถึงทรัพย์สิน หรือคิดฟุ้งซ่านต่าง ๆ นานา...",
        romanized:
          "Chit wela nang yu rue doen yu tha chit khit thueng ban khit thueng sap sin rue khit fungsan tang tang nana...",
        eng: "When the mind wanders — thinking about home, possessions, or other distractions — place attention at the solar plexus and note 'thinking... thinking...' until the thoughts fade.",
      },
      {
        thai: "ดีใจ เสียใจ หรือโกรธ ก็ให้กำหนดเช่นเดียวกัน ว่า ดีใจหนอ ๆ ๆ ๆ เสียใจหนอ ๆ ๆ ๆ โกรธหนอ ๆ ๆ ๆ เป็นต้น",
        romanized:
          "Di chai sia chai rue krot ko hai kamnot chen diao kan wa di chai no no no no sia chai no no no no krot no no no pen ton",
        eng: "When feeling joy, sadness, or anger, apply the same technique — noting 'happy... happy...', 'sad... sad...', or 'angry... angry...' accordingly.",
      },
      {
        thai: "เวลานอน ค่อย ๆ เอนตัวนอน พร้อมกับกำหนด เอาสติมาจับที่ท้อง แล้วกำหนดว่า พองหนอ ยุบหนอ...",
        romanized:
          "Wela non khoi khoi en tua non phrom kap kamnot ao sati ma chap thi thong laeo kamnot wa phong no yub no...",
        eng: "When lying down to sleep, gently recline and place mindfulness at the abdomen, noting 'rising... falling...' continuously until you fall asleep.",
      },
      {
        thai: "อิริยาบถต่าง ๆ การเดินไปในที่ต่าง ๆ การเข้าห้องน้ำ การเข้าห้องส้วม การรับประทานอาหาร และการกระทำกิจการงานทั้งปวง...",
        romanized:
          "Iriyabot tang tang kan doen pai nai thi tang tang kan khao hong nam kan khao hong suam kan rapprathan ahan lae kan kratham kitjakan ngan thang puang...",
        eng: "During all activities — walking, using the restroom, eating, and working — maintain mindfulness in every movement and posture.",
      },
      {
        thai: "ผู้ปฏิบัติต้องมีสติกำหนดอยู่ทุกขณะในอาการเหล่านี้ ตามความเป็นจริง คือมีสติสัมปชัญญะเป็นปัจจุบันอยู่ตลอดเวลา",
        romanized:
          "Phu patibat tong mi sati kamnot yu thuk khana nai akarn lao ni tam khwam pen ching khue mi sati sampachanya pen patjuban yu talot wela",
        eng: "A practitioner must remain mindful at all times in all postures and actions, staying present in the moment with continuous awareness.",
      },
    ],
  },
  {
    title: "อนุโมทนา",
    titleRomanized: "Anumodana",
    description: `
    This is a blessing and encouragement to recite prayers regularly as part of daily life.
    It emphasizes the spiritual and moral benefits of chanting for individuals, families,
    and society at large. It highlights how chanting brings wisdom, happiness, and discipline,
    especially to younger generations, and ends with a formal well-wishing by Phra Rajsuddhiyanmongkol.
  `,
    paragraphs: [
      {
        thai: "สวดมนต์ไหว้พระเป็นธรรมประจำชีวิต เป็นข้อคิดประจำชีวิต เกิดผลผลิเพื่องอกงามสร้างความดีแก่ตน ผลกำไรเป็นความดีเพื่อมอบให้แก่เพื่อนร่วมชาติ ร่วมโลกได้อยู่ด้วยความมีโชคดี ทุกๆ ท่าน",
        romanized:
          "Suat mon wai phra pen thammacham pracham chiwit pen khokhit pracham chiwit koet phonphali phuea ngok-ngam sang khwam di kae ton phonkamrai pen khwam di phuea mop hai kae phuean ruam chat ruam lok dai yu duai khwam mi chok di thuk thuk than",
        eng: "Reciting chants and paying homage to the Buddha should be a regular part of life. It serves as daily reflection, brings spiritual growth, and cultivates goodness within oneself. The benefit is not only personal but can be shared with fellow citizens and all beings in the world, bringing good fortune to everyone.",
      },
      {
        thai: "ขอให้ท่านพร้อมสมาชิกในครอบครัวได้สวดมนต์กันทุกคน ทุกครอบครัวเพื่อเป็นมงคลในชีวิต จะเกิดฐานะดีมีปัญญาจะได้มีความสุข ความเจริญยิ่ง ๆ ขึ้นไปในชีวิต",
        romanized:
          "Kho hai than phrom samachik nai khropkhruea dai suat mon kan thuk khon thuk khropkhruea phuea pen mongkhon nai chiwit cha koet thana di mi panya cha dai mi khwam suk khwam charoen ying ying khuen pai nai chiwit",
        eng: "May you and your family members all recite these chants together. This will bring auspiciousness into your lives, leading to prosperity, wisdom, happiness, and continued success.",
      },
      {
        thai: "ขอให้ท่านๆ ชายหญิงทั่วกัน ลูกหลานทุกๆ คนสวดมนต์ก่อนนอน ถ้าทั้งหลายตั้งใจ ศรัทธาเชื่อมั่น ลูกหลานได้สวดมนต์ตามหนังสือนี้แล้ว ผลที่ได้รับจากการสวดมนต์นั้น",
        romanized:
          "Kho hai than than chai ying thua kan luk lan thuk thuk khon suat mon kon non tha thang lai tangchai sattha chuea man luk lan dai suat mon tam nangsue ni laeo phon thi dai rap chak kan suat mon nan",
        eng: "May all men and women, and every child, recite these prayers before sleeping. If everyone does so with intention and faith, the benefits from these chants will be as follows:",
      },
      {
        thai: "๑. ลูกหลานจะมีระเบียบวินัยที่ดี",
        romanized: "Nueng. Luk lan cha mi rabieb winai thi di",
        eng: "1. Children will develop good discipline.",
      },
      {
        thai: "๒. ลูกหลานจะไม่เถียง จะเคารพเชื่อฟังพ่อแม่ เขาจะรู้ว่าเขาเป็นเด็กหรือผู้ใหญ่ จะวางตัวได้เหมาะสม",
        romanized:
          "Song. Luk lan cha mai thiang cha khao-rop chuea fang pho mae khao cha ru wa khao pen dek rue phu yai cha wang tua dai mosom",
        eng: "2. Children will not be argumentative; they will respect and obey their parents, know their place, and behave appropriately.",
      },
      {
        thai: "๓. เมื่อเจริญวัยเป็นหนุ่มสาว ก็จะเป็นลูกหลานที่ดีของพ่อแม่ เป็นพลเมืองที่ดีของสังคมและประเทศชาติ",
        romanized:
          "Sam. Muea charoen wai pen num sao ko cha pen luk lan thi di khong pho mae pen phonlamueang thi di khong sangkhom lae prathet chat",
        eng: "3. As they grow into young adults, they will become good children to their parents and responsible citizens of society and the nation.",
      },
      {
        thai: "๔. ผู้ที่สวดและปฏิบัติเป็นประจำ จะเจริญรุ่งเรือง พัฒนาสถาพร จะรวย จะสวย จะดี มีปัญญา จะสมประสงค์ในสิ่งที่ดีงามตลอดไปทุกประการ",
        romanized:
          "Si. Phu thi suat lae patibat pen pracham cha charoen rung rueang patthana sathaphon cha ruai cha suai cha di mi panya cha som prasong nai sing thi di ngam talot pai thuk prakan",
        eng: "4. Those who chant and practice regularly will flourish, develop lasting stability, gain wealth, beauty, goodness, wisdom, and fulfill all wholesome aspirations permanently.",
      },
      {
        thai: "ขออำนวยพร",
        romanized: "Kho amnuai phon",
        eng: "May you be blessed.",
      },
      {
        thai: "พระราชสุทธิญาณมงคล วัดอัมพวัน อ. พรหมบุรี จ. สิงห์บุรี",
        romanized:
          "Phra Ratchasuthiyanmongkhon Wat Amphawan, Phromburi District, Singburi Province",
        eng: "Phra Rajsuddhiyanmongkol, Wat Amphawan, Phromburi District, Singburi Province",
      },
    ],
  },
  {
    title: "อานิสงส์ของการสวดพระพุทธคุณ",
    titleRomanized: "Anisong Khong Kan Suat Phra Phutthakun",
    description: `
    This short teaching from Phra Ratchasuthiyanmongkhon encourages people to chant the Buddha's qualities (Phra Phutthakun) regularly.
    It explains that instead of turning to astrologers or fortune tellers for bad luck,
    one can use chanting as a practical, spiritual remedy.
    Chanting should be done according to one's age (or more) to improve mindfulness, clarity, and inner stability.
  `,
    paragraphs: [
      {
        thai: "อานิสงส์ของการสวดพระพุทธคุณ พระราชสุทธิญาณมงคล",
        romanized:
          "Anisong Khong Kan Suat Phra Phutthakun Phra Ratcha Sutthiyanmongkhon",
        eng: "Merits of chanting the Buddha's qualities – Phra Ratchasuthiyanmongkhon",
      },
      {
        thai: "พระพุทธคุณอาตมาสังเกตมาว่า บางคนเขาไปหาหมอดู เคราะห์ร้ายก็ต้องสะเดาะเคราะห์ อาตมาก็มาดูเหตุการณ์โชคลางไม่ดีก็เป็นความจริงของหมอดู",
        romanized:
          "Phra Phutthakun atama sangket ma wa bang khon khao pai ha mo du khro r rai ko tong sada khro atama ko ma du hetkan choklang mai di ko pen khwam ching khong mo du",
        eng: "I've observed that some people go to fortune tellers when faced with bad luck and are told they must perform rituals to remove it. I too examined situations of bad omens and found that there is truth in what they describe.",
      },
      {
        thai: "อาตมาก็ตั้งตำราขึ้นมาด้วยสติ บอกว่าโยมไปสวดพุทธคุณเท่าอายุให้เกินกว่า ๑ ให้ได้ เพื่อให้สติดีเท่าที่ใช้ได้ผล",
        romanized:
          "Atama ko tang tamra khuen ma duai sati bok wa yom pai suat Phutthakun thao ayu hai koen kwa nueng hai dai phuea hai sati di thao thi chai dai phon",
        eng: "So I created a guideline using mindfulness: I told laypeople to chant the Buddha's qualities as many times as their age plus one, in order to cultivate stronger mindfulness that yields real results.",
      },
      {
        thai: "สวดตั้งแต่ นะโมพุทธัง ธรรมมัง สังฆัง พุทธคุณ ธรรมคุณ สังฆคุณ พาหุงมหากา",
        romanized:
          "Suat tangtae Namo Phutthang Thammamang Sanghang Phutthakun Thammakun Sangkhakun Pahung Mahaka",
        eng: "Begin chanting from: Namo Buddhāya, Dhammāya, Saṅghāya; followed by Buddha's, Dhamma's, and Sangha's qualities; and the Pahung Mahākā chant.",
      },
      {
        thai: "จบแล้วย้อนกลับมาข้างต้น เอาพุทธคุณห้องเดียว ห้องละ ๑ จบ ต่อ ๑ อายุ ๔๐ สวด ๔๑ อายุ ๓๕ สวด ๓๖ ก็ได้ผล",
        romanized:
          "Chop laeo yon klap ma khang ton ao Phutthakun hong diao hong la nueng chop tor nueng ayu si sip suat si sip et ayu sam sip ha suat sam sip hok ko dai phon",
        eng: "After finishing, return to the beginning and recite only the Buddha's qualities — one round per room, one chant per year of age. For example: if you are 40, chant 41 times; age 35, chant 36 times — it will bring results.",
      },
    ],
  },
  {
    title: "ชาวคริสต์กับพระพุทธคุณ",
    titleRomanized: "Chao Khrist Kap Phra Phutthakun",
    description: `
    This story recounts a real-life encounter with a Christian woman who, despite her religious background,
    began chanting Buddhist protective verses to help her troubled son succeed in school.
    It emphasizes that faith, perseverance, and chanting can transform lives, even healing emotional
    and physical struggles. The story also reflects on the power of a mother's merit and compassion.
  `,
    paragraphs: [
      {
        thai: "มีชาวคริสต์คนหนึ่ง อยู่แถวลาดพร้าว อายุ ๕๑ ปี มีลูกชายคนเดียว สามีตาย เป็นเศรษฐีที่ กทม.",
        romanized:
          "Mi chao Khrist khon neung yu thaew Lat Phrao, ayu ha-sip-et pi, mi luk chai khon diao, sami tai, pen setthi thi Krung Thep.",
        eng: "There was a Christian woman living in Lat Phrao, 51 years old, with one son. Her husband had passed away and she was a wealthy woman in Bangkok.",
      },
      {
        thai: "ราชาที่ดิน ที่ดินข้างคลองแสนแสบของเขาทั้งนั้นไปจรดลาดพร้าวหลายร้อยไร่...",
        romanized:
          "Racha thi din, thi din khang khlong Saen Saep khong khao thang nan pai charot Lat Phrao lai roi rai...",
        eng: "She owned land along the Saen Saep canal, stretching all the way to Lat Phrao, totaling hundreds of acres...",
      },
      {
        thai: "เมื่อลูกเรียนไม่เก่งก็ส่งลูกไปเรียนเมืองนอก ไปเรียนปริญญาที่อเมริกา...",
        romanized:
          "Muea luk rian mai keng ko song luk pai rian mueang nok, pai rian parinya thi America...",
        eng: "When her son didn't do well in school, she sent him abroad to study for a degree in America...",
      },
      {
        thai: "ลูกใช้เงินเยอะ เรียนไม่จบ สุดท้ายหาหมอดู หมอดูให้สะเดาะเคราะห์...",
        romanized:
          "Luk chai ngoen yoe, rian mai chop, sutthai ha mo du, mo du hai sadok khro...",
        eng: "Her son spent a lot of money, didn't finish his studies, and eventually she turned to a fortune teller who told her to perform rituals to remove bad luck...",
      },
      {
        thai: "อาตมาเจอเขาโดยบังเอิญ เขาไม่อยากแวะวัด แกล้งบอกว่าปวดท้องเพื่อเข้าห้องน้ำ...",
        romanized:
          "Atama choe khao doi bang-oen, khao mai yak wae wat, klaeng bok wa puat thong phuea khao hong nam...",
        eng: "I happened to meet them by chance. They didn't want to stop at the temple, pretending to need the restroom...",
      },
      {
        thai: "อาตมาไม่รู้ว่าเขาเป็นคริสต์ ขอให้ช่วยเพราะลูกชายไม่เรียน บอกว่าเป็นโรคประสาทแล้ว...",
        romanized:
          "Atama mai ru wa khao pen Khrist, kho hai chuai phro luk chai mai rian, bok wa pen rok prasan laeo...",
        eng: "I didn't know they were Christian. They asked for help because the son wasn't studying and she feared he had a nervous breakdown...",
      },
      {
        thai: "อาตมาดูหน้าแล้วรู้ว่าลูกชายต้องสำเร็จ แต่ทำไมยังไม่จบปริญญา อาตมาเสนอให้สวดมนต์...",
        romanized:
          "Atama du na laeo ru wa luk chai tong samret, tae thammai yang mai chop parinya, atama sanoe hai suat mon...",
        eng: "From looking at his face, I knew the son would succeed, but why hadn't he graduated yet? I suggested she try chanting...",
      },
      {
        thai: "โยมบอกว่าสวดไม่ได้เพราะเป็นคริสต์ อาตมาจึงแนะนำให้อ่านตามตัว ใช้ไม้ขีดนับรอบ...",
        romanized:
          "Yom bok wa suat mai dai phro pen Khrist, atama chuai nae nam hai an tam tua, chai mai khit nap rop...",
        eng: "She said she couldn't chant because she was Christian. I advised her to just read the text and use matchsticks to count repetitions...",
      },
      {
        thai: "หลังจากสวดได้ 3 เดือน เริ่มจำได้ ไม่ต้องใช้ไม้ขีดอีก เกิดอานิสงส์ 2 ประการ...",
        romanized:
          "Lang jak suat dai sam duean, roem jam dai, mai tong chai mai khit ik, koet anisong song prakarn...",
        eng: "After three months of chanting, she could recite it from memory without the matchsticks. Two major benefits emerged...",
      },
      {
        thai: "หนึ่ง หายโรคประสาท ใจดีขึ้น นอนหลับดี เริ่มแผ่เมตตาให้ลูก...",
        romanized:
          "Neung: hai rok prasan, chai di khuen, non lap di, roem phae metta hai luk...",
        eng: "First, her nervous disorder disappeared, her mood improved, she slept well, and began radiating loving-kindness to her son...",
      },
      {
        thai: "อีก 6 เดือน ลูกชายประสบอุบัติเหตุ รถชนเสาไฟฟ้า เพื่อนกระเด็นออกมาแต่ไม่เป็นไร...",
        romanized:
          "Ik hok duean, luk chai prasop u-bat-ti-het, rot chon sao faifa, phuean kraden ok ma tae mai pen rai...",
        eng: "Six months later, her son was in a car accident. The car hit a power pole. His friends were thrown out unharmed...",
      },
      {
        thai: "ลูกต้องรักษาตัว พี่ชายที่เป็นหมอในอเมริกาบอกว่าไม่รอดแน่...",
        romanized:
          "Luk tong raksa tua, phi chai thi pen mo nai America bok wa mai rot nae...",
        eng: "Her son had to be hospitalized. A cousin who was a doctor in America said he wouldn't survive...",
      },
      {
        thai: "วันรุ่งขึ้นลูกฟื้นตัว น้ำตาไหล คิดถึงแม่ รู้ซึ้งถึงบุญคุณของแม่...",
        romanized:
          "Wan rung khuen luk fuen tua, nam ta lai, khit thueng mae, ru sueng thueng bun khun khong mae...",
        eng: "The next day, her son woke up with tears in his eyes, finally realizing his mother's love and merit...",
      },
      {
        thai: "ลูกชายกลับมาวัด มาวิปัสสนา ไม่ยอมกลับอเมริกา...",
        romanized:
          "Luk chai klap ma wat, ma vipassana, mai yom klap America...",
        eng: "The son returned to the temple, practiced vipassana, and refused to go back to America...",
      },
      {
        thai: "แม่บอกว่าอาตมาช่วยไว้ สุดท้ายลูกสวดมนต์เอง ตัดผม เปลี่ยนพฤติกรรม...",
        romanized:
          "Mae bok wa atama chuai wai, sutthai luk suat mon eng, tat phom, plian phruekkatham...",
        eng: "The mother said I had saved them. Eventually, the son began chanting on his own, cut his hair, and changed his behavior...",
      },
      {
        thai: "บทสรุป: ถ้าใครเคราะห์ร้ายหรือดวงไม่ดี ให้สร้างพระเข้าตัว สวดมนต์เป็นยาทา วิปัสสนาเป็นยากิน",
        romanized:
          "Bot sarup: tha khrai khro rai rue duang mai di, hai sang Phra khao tua, suat mon pen ya tha, vipassana pen ya kin.",
        eng: "In conclusion: if someone has bad luck or misfortune, let them build Buddha into themselves. Chanting is like a balm; meditation is like medicine.",
      },
    ],
  },
  {
    title: "พาหุงมหากาและนิมิตจากสมเด็จพระพนรัตน์",
    titleRomanized: "Phāhung Mahākā læ Nimit chāk Somdet Phra Phonarat",
    description:
      "En berättelse om en vision från Somdet Phra Phonarat som ledde till spridningen av den kraftfulla bönen 'Phāhung Mahākā'. Kapitlet beskriver hur bönen ger seger, skydd och andlig kraft – både i kunglig historia och för dagens utövare.",
    paragraphs: [
      {
        thai: "เมื่ออาตมาได้พบกับ สมเด็จพระพนรัตน์ วัดป่าแก้ว คืนวันหนึ่งอาตมานอนหลับแล้วฝันไปว่า อาตมาได้เดินไปในสถานที่แห่งหนึ่งพบกับพระสงฆ์รูปหนึ่งครองจีวรคร่ำ สมณสารูปเรียบร้อยน่าเลื่อมใส",
        romanized:
          "Muea ātamā dai phop kap Somdet Phra Phonarat Wat Pa Kaew, kheun wan nueng ātamā non lap læo fan pai wā ātamā dai doen pai nai sathanthi hæng nueng, phop kap phra song rūp nueng khrong jīwon khram, samanasārūp riaproi nā lueam sai.",
        eng: "When I encountered Somdet Phra Phonarat of Wat Pa Kaew, one night I dreamed that I was walking in a place where I met a monk in worn robes with a dignified and inspiring appearance.",
      },
      {
        thai: 'อาตมาเห็นว่าเป็นพระอาวุโสผู้รัตตัญญู จึงน้อมนมัสการท่าน ท่านหยุดยืนตรงหน้าอาตมา แล้วกล่าวกับอาตมาว่า "ฉันคือสมเด็จพระพนรัตน์ วัดป่าแก้วแห่งกรุงศรีอยุธยา ฉันต้องการให้เธอได้ไปที่วัดใหญ่ชัยมงคลเพื่อดูจารึกที่ฉันได้จารึกถวายพระเกียรติแก่สมเด็จพระนเรศวรมหาราชผู้เป็นเจ้า"',
        romanized:
          'Ātamā hen wā pen phra āwuso phū rattanyu, jeung nôm namatsakān than. Than yūt yūn trong nā ātamā læo glāw kap ātamā wā "Chan khue Somdet Phra Phonarat Wat Pa Kaew hæng Krung Si Ayutthaya. Chan tong kān hai thoe dai pai thī Wat Yai Chai Mongkhon phuea dū jārik thī chan dai jārik thawai phra kīat kæ Somdet Phra Naresuan Mahārāt phū pen chāo."',
        eng: 'I realized he was a senior monk with deep insight, so I respectfully greeted him. He stood directly in front of me and said, "I am Somdet Phra Phonarat of Wat Pa Kaew in Ayutthaya. I want you to go to Wat Yai Chai Mongkhon and look at the inscription I carved in honor of King Naresuan the Great."',
      },
      {
        thai: 'ประเทศไทยจากหงสาวดีเป็นครั้งแรก เธอไปดูไว้ แล้วจดจํามาเผยแพร่ออกไป ถึงเวลาที่เธอจะได้รับรู้แล้ว"',
        romanized:
          "Prathet Thai jāk Hongsawadi pen khrang ræk. Thoe pai dū wai læo jot jam mā phoey phrae ōk pai. Thueng wela thī thoe cha dai rap rū læo.",
        eng: '"It was the first time Thailand gained independence from Hongsawadi. Go and study it, remember it, and spread it. Now is the time for you to understand."',
      },
      {
        thai: "ในฝันอาตมารับปากท่าน ท่านก็บอกตำแหน่งให้แล้วก็ตกใจตื่นนอนตอนใกล้รุ่ง อาตมาก็ทบทวนความฝัน ก็นึกอยู่ในใจว่าเราเองนั้นกำหนดจิตด้วยกรรมฐาน มีสติอยู่เสมอ เรื่องฝันฟุ้งซ่านเป็นไม่มี",
        romanized:
          "Nai fan ātamā rap pāk than. Than kō bōk tamnaeng hai læo kō tokjai tūen non tŏn klai rūng. Ātamā kō thopthuān khwām fan, kō nuek yū nai jai wā rao eng nán kamnot chit dûai kammathān mī sati yū samœ. Rūang fan fúngsān pen mai mī.",
        eng: "In the dream, I agreed to his request. He showed me the location, and I suddenly woke up at dawn. I reflected on the dream and thought: I always practice mindfulness meditation, so such a scattered dream is unusual for me.",
      },
      {
        thai: "อาตมาก็ได้ข่าวในวันนั้นแหละว่า ทางกรมศิลปากรทำการบูรณะปฏิสังขรณ์พระเจดีย์ใหญ่ในวัดใหญ่ชัยมงคล และจะทำการบรรจุบัวยอดพระเจดีย์ อันเป็นนิมิตหมายการสิ้นสุดการบูรณะ แล้วจะรื้อนั่งร้านทั้งหมดออกเป็นการเสร็จสิ้น",
        romanized:
          "Ātamā kō dai khāo nai wan nán læ wa thāng krom sinlapākon tham kān būrana patisangkōn phra chedī yai nai Wat Yai Chai Mongkhon læ ja tham kān banju bua yōt phra chedī an pen nimit māi kān sin sut kān būrana læ ja rʉ̄ nāng rāan thang mot ōk pen kān set sin.",
        eng: "That same day, I heard the news that the Fine Arts Department was restoring the large stupa at Wat Yai Chai Mongkhon, and that they would enshrine the finial as the final act of the renovation, after which all scaffolding would be removed.",
      },
      {
        thai: "อาตมาจึงได้ขอร้อง ดร.กิ่งแก้ว อัตถากร ให้เลื่อนการปิดยอดบัวไปอีกวันหนึ่ง เพื่อที่อาตมาจะได้นำพระชุมเสมาชัย ซุ้มเสมาขอ ที่อาตมาได้สร้างขึ้นตามแบบดั้งเดิมที่พบในเจดีย์ใหญ่ใกล้กับวัดอัมพวัน ซึ่งพังลงน้ำ ที่กึงเหล็งเป็นคนรวบรวมเอามาให้อาตมาตั้งแต่เมื่อเริ่มพัฒนาวัดใหม่ ๆ แต่แตกหักผุพังทั้งนั้น หลายสิบปีแล้ว อาตมาได้ป่นเอามาผสมสร้างเป็นองค์พระใหม่ ไปร่วมบรรจุไว้ที่ยอดพระเจดีย์บ้าง",
        romanized:
          "Ātamā jeung dai khōr rōng Dr. Kingkaew Atthākorn hai lʉ̄an kān pit yōt bua pai ik wan nʉeng, phʉ̄a thī ātamā cha dai nam Phra Chum Sema Chai, sum semā khō, thī ātamā dai sāng khʉn tām bǣp dang dœ̄m thī phop nai chedī yai klai kap Wat Ampha Wan, sʉ̄ng phang long nām, thī Kueng Leng pen khon ruapruam ao mā hai ātamā tangtǣ mʉ̄a rœ̄m phatthanā wat mai mai, tǣ taek hak phu phang thang nán, lāi sip pī lǣo. Ātamā dai pon ao mā phasom sāng pen ong Phra mai pai ruam banju wai thī yōt phra chedī bāng.",
        eng: "I therefore asked Dr. Kingkaew Atthakorn to postpone the enshrinement of the lotus finial by one day, so I could bring the Phra Chum Sema Chai—ornamental boundary markers—I had recreated based on the original style found in the large stupa near Wat Ampha Wan, which had collapsed into the river. Kueng Leng had collected the fragments for me decades ago when I began restoring the monastery. I had ground and mixed them to form a new Buddha image, to be enshrined within the stupa's pinnacle.",
      },
      {
        thai: "วันนั้นอาตมาได้เดินทางไปถึง ก็ได้เดินขึ้นไปบน แล้วมองเห็นโพรงที่ทางเขาเจดีย์ตอนที่สุดบ ทำไว้สำหรับลงไปด้านล่าง มีรานไม้พอไต่ลงไปภายใน ตั้งใจเด็ดเดี่ยวว่าลงไปคราวนี้ ถ้าพลาดตกลงไปจากนั่งร้านไม้ก็ยอมตาย",
        romanized:
          "Wan nán ātamā dai doen thāng pai thʉng, kō dai doen khʉn pai bon, læo mong hen phrōng thī thāng khao chedī ton thī sut ba tham wai samrap long pai dān lāng, mī rāan māi phō tai long pai phāi nai, tangjai detdīao wā long pai khrao nī, thā phlāt tok long pai jāk nāng rān māi kō yōm tāi.",
        eng: "That day I arrived and climbed up the stupa, where I saw a shaft at its uppermost part, made for descending inside. There was a wooden scaffold just enough to climb down. I was determined: if I were to fall from it, I would accept death.",
      },
      {
        thai: "คนที่ร่วมเดินทางมาด้วยเข้ามัวแต่ไปบนลานชั้นบน อาตมาก็ดิ่งลงไปชั้นล่าง มีไฟฉายดวงหนึ่ง เวลานั้นประมาณ ๐๙.๐๐ น. อาตมาลงไปภายใน แล้วก็พบนิมิตดังที่สมเด็จพระพนรัตน์ได้บอกไว้จริง ๆ",
        romanized:
          "Khon thī ruam doen thāng mā dûai khao mua tæ pai bon lān chan bon. Ātamā kō dīng long pai chan lāng, mī fai chāi duang nʉeng, wela nán bpramān kao mōng nā. Ātamā long pai phāi nai, læo kō phop nimit dang thī Somdet Phra Phonarat dai bōk wai jing jing.",
        eng: "Those who traveled with me were still walking around the upper platform, but I went straight down into the lower level with a flashlight. It was around 9:00 AM. Once I entered, I found the vision just as Somdet Phra Phonarat had told me.",
      },
      {
        thai: 'อาตมาจึงได้พบว่าแท้ที่จริงแล้ว สิ่งที่สมเด็จพระพนรัตน์ วัดป่าแก้วท่านได้จารึกถวายพระพร ก็คือบทสวดที่เรียกว่า "พาหุงมหาการุณิโก"',
        romanized:
          "Ātamā jʉng dai phop wā thæ thī jing læo, sing thī Somdet Phra Phonarat Wat Pa Kaew than dai jārik thawai phraporn, kō khue bot suat thī riak wā 'Phāhung Mahā Kārunikō'.",
        eng: "I then realized that what Somdet Phra Phonarat of Wat Pa Kaew had actually inscribed as a royal blessing was the chant known as 'Phahung Maha Karuniko'.",
      },
      {
        thai: 'ท้ายของนิมิตนั้นระบุว่า "เราสมเด็จพระพนรัตน์ วัดป่าแก้วศรีอโยธเยศ คือผู้จารึกนิมิต รจนาเอาไว้ถวายพระพรแด่มหาบพิตรเจ้าสมเด็จพระนเรศวรมหาราช"',
        romanized:
          "Thāi khong nimit nán rabū wā 'Rao Somdet Phra Phonarat Wat Pa Kaew Si Ayotthayēt khue phū jārik nimit rojana ao wai thawai phraporn dɛ̄ Mahābophit Chāo Somdet Phra Naresuan Mahārat.'",
        eng: "At the end of the vision, it stated: 'We, Somdet Phra Phonarat of Wat Pa Kaew Si Ayotthaya, are the ones who inscribed this vision and composed it as a royal blessing to His Majesty King Naresuan the Great.'",
      },
      {
        thai: "พาหุงมหากา ก็คือบทสวดสรรเสริญพระพุทธคุณ พระธรรมคุณ พระสังฆคุณ แล้วก็พระพาหุง อันเริ่มด้วย 'พาหุงสหัสส' ไปจนถึง 'ทุคาฮทิฏฐิ' แล้วเรื่อยไปจนถึง 'มหากรุณิโก นาโถ หิตายะ' และจบลงด้วย 'ภะวะตุสัพพะมังคะลัง สัพพะพุทธา สัพพะธัมมา สัพพะสังฆานุภาเวนะ สะทา โสตถี ภะวันตุ เต'",
        romanized:
          "Phāhung Mahākā kō khue bot suat sansern Phra Phutthakhun, Phra Thammakhun, Phra Sangkhakhun, læo kō Phra Phāhung an rœ̄m dûai 'Phāhung sahassa' pai jon thueng 'Thukāhathitthi' læo rʉ̄ai pai jon thueng 'Mahā Karuniko Nātho Hitāya', læ chob long dûai 'Bhavatu Sabbha Maṅgalaṃ, Sabbha Buddhā Sabbha Dhammā Sabbha Saṅghānubhāvena Sadā Sotthī Bhavantu Te.'",
        eng: "The 'Phahung Mahākā' is a chant that praises the virtues of the Buddha, the Dhamma, and the Sangha, beginning with 'Phāhung sahassa…' up to 'Thukāhathitthi', continuing through 'Mahā Karuniko Nātho Hitāya', and concluding with 'Bhavatu Sabbha Maṅgalaṃ, Sabbha Buddhā, Sabbha Dhammā, Sabbha Saṅghānubhāvena, Sadā Sotthī Bhavantu Te.'",
      },
      {
        thai: "อาตมาเรียกรวมกันว่า 'พาหุงมหากา' อาตมาจึงเข้าใจในบัดนั้นเองว่า บทพาหุงนี้คือบทสวดมนต์ที่สมเด็จพระพนรัตน์ วัดป่าแก้ว ได้ถวายให้พระบาทสมเด็จพระนเรศวรมหาราชไว้สวดเป็นประจำ เวลาอยู่กับพระมหาราชวัง และในระหว่างศึกสงคราม",
        romanized:
          "Ātamā riak ruam kan wā 'Phāhung Mahākā'. Ātamā jʉng khaochai nai bat nán eng wā bot Phāhung nī khue bot suatmon thī Somdet Phra Phonarat Wat Pa Kaew dai thawai hai Phra Bāt Somdet Phra Naresuan Mahārat wai suat pen pracham wela yū kap Phra Mahārāchawang læ nai rawāng sʉk songkhrām.",
        eng: "I collectively referred to it as 'Phahung Mahākā'. I understood then that this chant was what Somdet Phra Phonarat of Wat Pa Kaew had given King Naresuan the Great to chant regularly, both while in the royal palace and during times of war.",
      },
      {
        thai: "จึงปรากฏว่าพระบาทสมเด็จพระนเรศวรมหาราชเจ้าทรงรบ ณ ที่ใด ทรงมีชัยชนะอยู่ตลอดมามิได้ทรงเพลี่ยงพล้ำเลย แม้จะเพียงลำพังสองพระองค์กับสมเด็จพระอนุชาธิราชเจ้า ท่ามกลางกองทัพพม่าจำนวนนับแสนคน ก็ทรงมีชัยชนะเหนือกองทัพพม่าด้วยการกระทำยุทธหัตถี มีชัยเหนือพระมหาอุปราชา ณ ดอนเจดีย์ ปูชนียสถาน",
        romanized:
          "Jʉng prākot wā Phra Bāt Somdet Phra Naresuan Mahārat Chāo song rop na thī dai, song mī chai chana yū talōt mā mi dai song plīang phlām lœ̄i. Mæ chā phiang lampang sōng Phra Ong kap Somdet Phra Anuchāthirāt Chāo, thām klāng kong thap Phamā jamnuan nap sæn khon, kō song mī chai chana nʉ̄a kong thap Phamā dûai kān kratham yutthahatti, mī chai nʉ̄a Phra Mahā Upparātā na Don Chedī, pūchaniyasathān.",
        eng: "Thus, it became evident that wherever King Naresuan the Great fought, he always emerged victorious without ever being defeated. Even when it was just him and his younger brother against the massive Burmese army of hundreds of thousands, he still prevailed — winning a war elephant duel over the Crown Prince of Burma at Don Chedi, a revered historical site.",
      },
      {
        thai: "แม้ข้าศึกจะยิงปืนไฟเข้าใส่พระองค์ในตอนที่เข้ากันพระศพของพระมหาอุปราชาออกไปราวกับห่าฝนก็มิปาน แต่ก็มิได้ต้องพระองค์ ด้วยเดชะพาหุงมหากาที่ทรงเจริญอยู่เป็นประจำนั่นเอง",
        romanized:
          "Mæ khāsʉk ja ying pʉ̄n fai khao sai Phra Ong nai ton thī khao kan Phra Soph khong Phra Mahā Upparātā ōk pai rāo kap hā fon kō mi pān, tæ kō mi dai tong Phra Ong. Dūai dēcha Phāhung Mahākā thī song jaroen yū pen pracham nán eng.",
        eng: "Even when the enemy fired volleys of gunfire at His Majesty during the procession of the Crown Prince's remains—like rainstorms—it did not harm him. It was due to the power of the Phahung Mahākā, which he had regularly recited.",
      },
      {
        thai: 'อาตมาพบนิมิตแล้ว ก็ไต่ขึ้นมาด้วยความสบายใจถึงปากปล่องที่ลงไป ใช้เวลาเกือบสามชั่วโมง เนื้อตัวมีแต่หยากไย่ เดินลงมาแม่ชีเห็นเข้ายังร้องว่า "หลวงพ่อเข้าไปในโพรงนั้นมาหรือ" แต่อาตมาไม่ตอบ',
        romanized:
          "Ātamā phop nimit læo, kō tai khʉn mā dûai khwām sabāi jai thʉng pāk plòng thī long pai, chai wela kʉ̄ap sām chūamōng. Nʉ̄a tūa mī tæ yāk yai. Doen long mā, mæ chī hen khao yang rōng wā 'Luang Phō khao pai nai phrōng nán mā rʉ̄?' Tæ ātamā mai tōp.",
        eng: "After discovering the vision, I climbed back up calmly to the mouth of the shaft I had descended—having spent nearly three hours inside. My body was covered in cobwebs. As I came down, a nun saw me and exclaimed, 'Did you go into that hole, Venerable Father?' But I did not reply.",
      },
      {
        thai: "ตั้งแต่นั้นมา อาตมาจึงสอนการสวดพาหุงมหากาให้แก่ญาติโยมเป็นต้นมา เพราะอะไร เพราะพาหุงมหากานั้นเป็นบทสวดมนต์ที่มีค่าที่สุด มีผลดีที่สุด เพราะเป็นชัยชนะอย่างสูงสุดของพระบรมศาสดา",
        romanized:
          "Tâng tǣ nán mā, ātamā jʉng sōn kān suat Phāhung Mahākā hai kɛ̄ yāt yōm pen ton mā. Phro àrai? Phro Phāhung Mahākā nán pen bot suatmon thī mī khā thī sūt, mī phon thī dī thī sūt, phro pen chai chana yāng sūng sūt khōng Phra Borommasātadā.",
        eng: "From that time on, I began teaching laypeople to recite the Phahung Mahākā. Why? Because this chant is the most valuable and effective one—it represents the Supreme Victory of the Great Teacher, the Buddha.",
      },
      {
        thai: "จากพญาวัสวดีมาร จากอาฬาวกะยักษ์ จากช้างนาฬาคิรี จากองคุลีมาล จากนางจิญจมาณวิกา จากสัจจะกะนิครนธ์ จากพญานันโทปนันทนาคราช และท่านท้าวผกาพรหม",
        romanized:
          "Chāk Phayā Wasawadī Mān, chāk Ālāwakayak, chāk Chāng Nālākhirī, chāk Ongkulīmāl, chāk Nāng Chinchāmanawikā, chāk Sajjakanikron, chāk Phayā Nanthōpanan Nākhārāt, læ than Thāo Phakhāphrom.",
        eng: "From the demon king Wasawadee, the ogre Āḷavaka, the elephant Nāḷāgiri, the bandit Angulimāla, the slander by Lady Cincā Māṇavikā, the ascetic Saccaka, the nāga king Nandopananda, and even the Brahma god Phakkhaprom.",
      },
      {
        thai: "เป็นชัยชนะที่พระพุทธองค์ทรงได้มาด้วยอิทธิปาฏิหาริย์ และด้วยอำนาจแห่งบารมีธรรมโดยแท้ ผู้ใดได้สวดไว้เป็นประจำทุกวัน จะมีชัยชนะ มีความเจริญรุ่งเรืองตลอดกาลนาน มีสติ",
        romanized:
          "Pen chai chana thī Phra Phuttha Ong song dai mā dûai itthipātihān, læ dûai amnāt hɛ̀ng bāramītham dōi thǽ. Phū dai dai suat wai pen pracham thúk wan, ja mī chai chana, mī khwām jarœ̄n rūng rʉ̄ang talōt kān nān, mī sati.",
        eng: "These were victories the Buddha attained through miraculous powers and the strength of His spiritual perfections. Whoever recites this chant regularly each day will enjoy victory, prosperity, and mindfulness throughout time.",
      },
      {
        thai: "ระลึกได้ จะตายก็ไปสู่สุคติภูมิ ขอให้ญาติโยมสวดพาหุงมหากากันให้ทั่วหน้า นอกจากจะคุ้มตัวแล้วยังคุ้มครองครอบครัวได้",
        romanized:
          "Ralʉk dai, ja tāi kō pai sū sukhatiphūmi. Khō hai yāt yōm suat Phāhung Mahākā kan hai thùa nā. Nòk jāk ja khūm tua læo yang khūm khrōng khrōpkhrua dai.",
        eng: "With mindfulness, even at the moment of death, one will reach a fortunate realm. May all laypeople chant Phahung Mahākā widely—not only for self-protection, but also for the protection of their families.",
      },
      {
        thai: "สวดมาก ๆ เข้า สวดกันทั้งประเทศ ก็ทำให้ประเทศมีแต่ความรุ่งเรือง พวกคนพาลสันดานหยาบก็แพ้ภัยไปอย่างถ้วนหน้า",
        romanized:
          "Suat mâk mâk khâo, suat kan thāng prathēt, kō tham hai prathēt mī tæ khwām rūng rʉ̄ang. Phûak khon phān sandān yāb kō phæ̂ phai pai yāng thûan nā.",
        eng: "When many people chant—if the whole nation does—it will bring prosperity to the country. Wicked and coarse-natured people will be overcome by their own misdeeds.",
      },
      {
        thai: "ไม่ใช่แต่พระบาทสมเด็จพระนเรศวรมหาราชเท่านั้น ที่พบความมหัศจรรย์ของบทพาหุงมหากา แม้พระบาทสมเด็จพระเจ้าตากสินมหาราชก็ทรงพบเช่นกัน",
        romanized:
          "Mai chai tæ Phra Bāt Somdet Phra Naresuan Mahārat thâonán thī phop khwām mahatsajan khōng bot Phāhung Mahākā. Mæ Phra Bāt Somdet Phra Chāo Taksin Mahārat kō song phop chên kan.",
        eng: "It wasn't only King Naresuan the Great who experienced the marvel of the Phahung Mahākā chant; even King Taksin the Great did as well.",
      },
      {
        thai: 'โดยมีบันทึกโบราณบอกไว้ว่าดังนี้ "เมื่อพระเจ้าตากสินมหาราชตีเมืองจันทบุรีได้แล้ว ก็ทรงเห็นว่าสงครามกู้ชาติต่อจากนี้ไปจะต้องหนักหนาและยืดยาว"',
        romanized:
          "Dōi mī banthʉk bōrān bōk wai wā dang nī: 'Mʉ̄a Phra Chāo Taksin Mahārat tī Mʉ̄ang Chanthaburī dai læo, kō song hen wā songkhrām kū chāt tō chak nī pai ja tông nakh nā læ yʉ̄t yāo.'",
        eng: "An ancient record states: 'When King Taksin the Great conquered Chanthaburi, he foresaw that the war to reclaim the nation would become heavier and prolonged from that point on.'",
      },
      {
        thai: "จึงทรงโปรดเกล้าฯ ให้สร้างพระยอดธงแบบศรีอยุธยาขึ้น แล้วนิมนต์พระเถระทั้งหลายมาสวดบทพาหุงมหากา บรรจุไว้ในองค์พระ",
        romanized:
          "Jʉng song prōt klâo hâi săng Phra Yōt Thong bǣp Sī Ayutthayā khʉ̂n, læo nimon Phra Thera thang lǎi mā suat bot Phāhung Mahākā, banju wai nai ong Phra.",
        eng: "So he graciously ordered the creation of a Phra Yot Thong Buddha statue in the Ayutthaya style, and invited senior monks to chant the Phahung Mahākā and enshrine it within the statue.",
      },
      {
        thai: "และพระองค์ก็ทรงเจริญรอยตามพระบาทสมเด็จพระนเรศวรมหาราชด้วยการเจริญพาหุงมหากา จึงบันดาลให้ทรงกู้ชาติสำเร็จ",
        romanized:
          "Læ Phra Ong kō song jarœ̄n roi tām Phra Bāt Somdet Phra Naresuan Mahārat dûai kān jarœ̄n Phāhung Mahākā. Jʉng bandān hâi song kū chāt samret.",
        eng: "And His Majesty followed in the footsteps of King Naresuan the Great by regularly reciting the Phahung Mahākā, which brought about his success in restoring the nation.",
      },
      {
        thai: "สวดพาหุงมหากากันให้ได้ทุกบ้าน สวดให้ได้มาก ๆ จะมีแต่ความเจริญรุ่งเรือง",
        romanized:
          "Suat Phāhung Mahākā kan hâi dai thúk bâan, suat hâi dai mâak mâak ja mī tæ khwām jarœ̄n rūng rʉ̄ang.",
        eng: "Let every household recite the Phahung Mahākā. The more it is recited, the more prosperity will arise.",
      },
      {
        thai: "สวดพาหุงมหากาก่อนแล้วจึงสวดชินบัญชร เพราะชินบัญชรนั้นเจ้าประคุณสมเด็จท่านใช้สวดบูชาพระอรหันต์ของท่าน",
        romanized:
          "Suat Phāhung Mahākā kòn læo jʉng suat Chinabanjorn, phro Chinabanjorn nán Chao Prakhun Somdet than chai suat būchā Phra Arahant khǒng than.",
        eng: "Recite the Phahung Mahākā first, then the Jinapanjara. The Jinapanjara was used by Somdet to worship his revered arahants.",
      },
      {
        thai: "ต้องสวดพาหุงมหากาก่อน แล้วจึงมาถึงชินบัญชร ให้จดจำกันเอาไว้นั่นแหละมงคลในชีวิต",
        romanized:
          "Tông suat Phāhung Mahākā kòn, læo jʉng mā thʉng Chinabanjorn. Hâi jòtjam kan ao wai nán læ mokkhon nai chīwit.",
        eng: "You must chant Phahung Mahākā first, and only then proceed to Jinapanjara. Remember this well—for it is a true blessing in life.",
      },
      {
        thai: "พระราชสุทธิญาณมงคล เจ้าอาวาสวัดอัมพวัน และเจ้าคณะอำเภอพรหมบุรี จังหวัดสิงห์บุรี",
        romanized:
          "Phra Rātchasutthiyānmongkhon, Chao Āwāt Wat Amphawan, læ Chao Khana Amphoe Phromburi, Changwat Singburi.",
        eng: "Phra Ratchasuddhiyanmongkhon, Abbot of Wat Amphawan and Ecclesiastical District Chief of Phromburi District, Singburi Province.",
      },
    ],
  },
];

export {
  characterTestADEPT15,
  testimonials,
  testCertificates,
  testAboutData,
  testDataJimmyCard,
  testEpigramData,
  testDeathNoteRules,
  testDataJapaneseZodiacSigns,
  chantbookPhraPhutthakun,
};
